...
/* 増減に色は付けない。一度「減っているときは赤」にしたが外した。
   今の運用方針は「新規獲得より既存リードの消化を優先」なので、減っている
   （消化が進んでいる）のは正常。赤は逆向きだった。かといってプラスを赤に
   するのも違う。符号の良し悪しは残りの水準で変わる（残り1,416での−180は
   正常、残り400での−180は警告）ので、固定の色では表せない。 */
...
def render(data):
    ...
    lead_box = (
        ...
        # 色は付けない。符号の良し悪しは残りの水準で変わるので、固定の色では
        # 表せない（残り1,416での−180は正常、残り400での−180は警告）。
        # 水準を見て色を変えるのは画面が判断を代行することになる。
        # マイナス記号だけで符号は読める。
        + lb_row(f"増減 {FLOW_WINDOW_DAYS}日",
                 f'{"+" if net > 0 else ""}{f_int(net)}')
        + "</aside>"
    )
